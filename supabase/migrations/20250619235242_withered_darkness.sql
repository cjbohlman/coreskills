/*
  ...
  1. New Learning Path
    ...
    - Icon: `zap` for performance representation
    - Order index: 6 (after existing paths)
...
class OptimizedDataService {
    ...
    async preloadUserNetwork(userId) {
        ...
        // Simulate fetching user friends/connections
...
class DatabaseService {
    ...
    async getProductsWithCategories(limit = 10) {
        ...
        const products = await this.db.query(
            "SELECT id, name, price, category_id FROM products LIMIT " + limit
        );
    ...
    async getProductsPaginated(page = 1, pageSize = 20) {
        ...
        return await this.db.query(
            "SELECT * FROM products ORDER BY created_at DESC OFFSET " + offset + " LIMIT " + pageSize
        );
    }
    ...
    async getOrderStatistics(userId) {
        ...
        const totalSpent = await this.db.query(
            "SELECT SUM(total_amount) as total FROM orders WHERE user_id = " + userId
        );
        
        this.queryCount++;
        const avgOrderValue = await this.db.query(
            "SELECT AVG(total_amount) as avg FROM orders WHERE user_id = " + userId
        );
    ...
    async getPopularProducts() {
        ...
        return await this.db.query(
            "SELECT p.*, COUNT(oi.product_id) as order_count FROM products p LEFT JOIN order_items oi ON p.id = oi.product_id LEFT JOIN orders o ON oi.order_id = o.id WHERE o.created_at > NOW() - INTERVAL 30 DAY GROUP BY p.id ORDER BY order_count DESC LIMIT 10"
        );
    }
...
class OptimizedDatabaseService {
    ...
    async getProductsWithCategories(limit = 10) {
        ...
        const products = await this.db.query(
            "SELECT p.id, p.name, p.price, p.category_id, c.name as category_name FROM products p INNER JOIN categories c ON p.category_id = c.id ORDER BY p.created_at DESC LIMIT $1",
            [limit]
        );
    ...
    async getProductsPaginated(cursor = null, pageSize = 20) {
        ...
        if (cursor) {
            ...
            query = "SELECT id, name, price, created_at FROM products WHERE created_at < $1 ORDER BY created_at DESC LIMIT $2";
            params = [cursor, pageSize];
        } else {
            // First page
            query = "SELECT id, name, price, created_at FROM products ORDER BY created_at DESC LIMIT $1";
            params = [pageSize];
        }
    ...
    async searchProducts(searchTerm) {
        ...
        const results = await this.db.query(
            "SELECT id, name, description, price, ts_rank(search_vector, plainto_tsquery($1)) as rank FROM products WHERE search_vector @@ plainto_tsquery($1) ORDER BY rank DESC, name ASC LIMIT 50",
            [searchTerm]
        );
    ...
    async getOrderStatistics(userId) {
        ...
        const stats = await this.db.query(
            "SELECT COUNT(*) as total_orders, COALESCE(SUM(total_amount), 0) as total_spent, COALESCE(AVG(total_amount), 0) as avg_order_value, MAX(created_at) as last_order_date FROM orders WHERE user_id = $1",
            [userId]
        );
    ...
    async createProductsBatch(products) {
        ...
        products.forEach((product, index) => {
            ...
            valuesClauses.push("($" + (baseIndex + 1) + ", $" + (baseIndex + 2) + ", $" + (baseIndex + 3) + ", $" + (baseIndex + 4) + ")");
            params.push(product.name, product.description, product.price, product.category_id);
        });
    ...
    async updateProductPricesBatch(priceUpdates) {
        ...
        priceUpdates.forEach((update, index) => {
            ...
            caseStatements.push("WHEN id = $" + (baseIndex + 1) + " THEN $" + (baseIndex + 2));
            params.push(update.id, update.price);
            ids.push(update.id);
        });
        
        const query = "UPDATE products SET price = CASE " + caseStatements.join(" ") + " END, updated_at = CURRENT_TIMESTAMP WHERE id = ANY($" + (params.length + 1) + ")";
        
        const result = await this.db.query(query, [...params, ids]);
...
class OptimizedMockDatabase {
    async query(sql, params = []) {
        ...
        return this.generateMockData(sql, params);
    }
    
    generateMockData(sql, params) {
