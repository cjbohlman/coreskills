/*
  # Add Code Review Learning Path

  1. New Learning Path
    - `Code Review Mastery` learning path for developing critical code review skills
    - Covers code quality, security, performance, and architectural issues

  2. Code Review Challenges
    - Progressive difficulty from beginner to advanced
    - Multiple programming languages and contexts
    - Interactive annotation system for identifying issues
    - Comprehensive test cases demonstrating problems

  3. Challenge Structure
    - Problematic code snippets with deliberate issues
    - Multiple issue categories (security, performance, maintainability, etc.)
    - Time-limited reviews to simulate real-world scenarios
    - Detailed explanations and fix recommendations
*/

-- Insert Code Review learning path
INSERT INTO learning_paths (id, title, description, icon, order_index) VALUES 
(
  gen_random_uuid(),
  'Code Review Mastery',
...
-- Get the learning path ID for challenges
DO $$
DECLARE
    path_id uuid;
BEGIN
    ...
    INSERT INTO challenges (id, title, description, difficulty, challenge_type, initial_code, solution_code, test_cases, path_id, order_index) VALUES 
    (
        gen_random_uuid(),
...
public class TransferService {
    ...
    public static void ProcessBatch(List<(int from, int to, decimal amount)> transfers) {
...
public class TransferService {
    ...
    public static async Task ProcessBatchAsync(List<(int from, int to, decimal amount)> transfers) {
        var tasks = transfers.Select(transfer => 
            Task.Run(() => Transfer(transfer.from, transfer.to, transfer.amount))
        );
        
        await Task.WhenAll(tasks);
    }
    
    // Safe way to get account balance
    public static decimal? GetAccountBalance(int accountId) {
        return _accounts.TryGetValue(accountId, out var account) ? account.Balance : null;
    }
}',
        '[
            {
            ...
            {
                "issue_type": "performance",
                "description": "Parallel.ForEach may create excessive contention",
                "line": "54-58",
                "severity": "medium",
                "fix": "Consider limiting parallelism or using async/await pattern"
            }
        ]',
        path_id,
        4
...
class OrderProcessor {
    ...
    private async checkInventoryAvailability(items: OrderItem[]): Promise<InventoryCheckResult> {
    ...
    private async createOrder(orderData: OrderData, total: number, transactionId: string): Promise<Order> {
        const order: Order = {
